...
Solution file: `Loyalty/Loyalty.sln`
...
Loyalty follows **Clean Architecture** with **CQRS-lite** via MediatR. Business logic lives in handlers; controllers and infrastructure adapters stay thin.
...
| CQRS-lite | Application | `ICommand`, `IQuery`, `*CommandHandler`, `*QueryHandler` |
...
| DTO Mapping (manual) | Application + Api | Domain → `Application.Models.Program` → `Api.Contracts.Program` |
...
| Kind | Type | Handler |
|---|---|---|
| Command | `CreateProgramCommand` | `CreateProgramCommandHandler` |
| Command | `UpdateProgramCommand` | `UpdateProgramCommandHandler` |
| Command | `DeleteProgramCommand` | `DeleteProgramCommandHandler` |
| Command | `ProcessTransactionCreatedCommand` | `ProcessTransactionCreatedCommandHandler` |
| Query | `GetProgramsQuery` | handler in same file |
| Query | `GetProgramQuery` | `GetProgramQueryHandler` |
...
|---|---|---|
| `_id` | string | Document identifier (program id) |
| `title` | string | Title |
| `description` | string? | Description |
| `state` | string | `Active` / `NotActive` |
| `startDate`, `finishDate` | datetime | Validity period |
| `minTransactionAmount`, `maxTransactionAmount` | decimal | Transaction amount range |
| `transactionType` | string | `Deposit` or `Withdrawal` |
| `achievement` | object | Embedded document (see below) |
...
| `transactionsCountToApplyAchievement` | int | Transactions required before reward |
| `reward.id` | string | Reward id |
| `reward.amount` | decimal | Percent or fixed amount |
| `reward.type` | string | `Percent` / `Fixed` |
| `reward.target` | string | `Bonus` |
| `reward.usageType` | string | `Add` |

### Collection `transactions`

| Field | Type | Description |
|---|---|---|
| `_id` | string | Document identifier (= WebMoney transaction id) |
| `eventId` | uuid | Kafka event id |
| `rrn` | string | RRN |
| `cardId` | int | Customer card |
| `amount` | decimal | Amount |
| `transactionType`, `transactionStatus` | string | Type and status |
| `categoryId` | int? | Category |
| `programId` | string? | Logical reference to `programs._id` (matched program) |
| `bonusAmountToAccrue` | decimal? | Calculated bonus |
| `createdAt`, `updatedAt` | datetime | Audit |
| `createdBy` | string | Source (system / event) |
| `receivedAt` | datetime | When Loyalty received the event |

### Collection `participations`

| Field | Type | Description |
|---|---|---|
| `_id` | string | Document identifier (`{cardId}:{programId}`) |
| `cardId` | int | Card |
| `programId` | string | Logical reference to `programs._id` |
| `qualifyingTransactionCount` | int | Qualifying transaction counter |

### Collection `bonus_outbox`

| Field | Type | Description |
|---|---|---|
| `_id` | string | Document identifier (= source transaction id) |
| `cardId` | int | Card |
